...
public class ImageCropActivity extends ImageBaseActivity implements View.OnClickListener, CropImageView.OnBitmapSaveCompleteListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        handleRotateOnSamsung(mImageItems);
...
//        Toast.makeText(ImageCropActivity.this, "裁剪成功:" + file.getAbsolutePath(), Toast.LENGTH_SHORT).show();
     ...
     * 处理三星设备图片旋转的问题
     *
     * @param mImageItems 图片集合
     */
    private void handleRotateOnSamsung(ArrayList<ImageItem> mImageItems) {
        for (ImageItem mImageItem : mImageItems) {
            Bitmap bitmap = BitmapFactory.decodeFile(mImageItem.path);

            /*处理三星设备上图片翻转90度的问题*/
            //读取图片exif 获得图片旋转角度
            int degree = ImageUtils.readPictureDegree(mImageItem.path);
            //翻转图片,拿到翻转后的正确角度的图片
            Bitmap turned_bitmap = ImageUtils.rotaingImageView(degree, bitmap);
            /*处理三星设备上图片翻转90度的问题*/

            //获取新图path
            File file = ImageUtils.saveBitmapToFile2(
                    this, turned_bitmap, turned_bitmap.getWidth(), turned_bitmap.getHeight(), this.getCacheDir().getAbsolutePath());
            String path = file.getAbsolutePath();

            //覆盖原来的path
            mImageItem.path = path;
